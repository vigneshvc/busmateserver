...
$sql = "SELECT `student_id`, `student_loginid`, `passhash`, `stop_id`, `pref_bus_id` FROM `student` WHERE student_id=".$mailid;
$retval=mysqli_query($conn, $sql);  
	if(mysqli_num_rows($retval) >0){
	//existing user

		
	$query="UPDATE `student` SET `passhash`='".md5($password)."',`stop_id`=".$stopid.",`pref_bus_id`=".$busid." WHERE student_loginid='".$mailid."';";
	if(mysqli_query($conn,$query)){
		...
		$query="INSERT INTO `student`(`student_loginid`, `passhash`, `stop_id`, `pref_bus_id`) VALUES ('".$mailid."','".md5($password)."',".$stopid."
		,".busid.")";
	if(mysqli_query($conn,$query)){
		$response['status']="success";
	}else{
